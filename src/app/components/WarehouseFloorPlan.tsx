...
function formatLocation(loc: SelectedLocation) {
  if (!loc.level) return `${loc.row}-${loc.column}`;
  return `${loc.row}-${loc.column}-${loc.level}`;
}
...
export function WarehouseFloorPlan() {
  ...
  const crosshairOn = Boolean(selected?.level);
  const highlightedRow = selected?.row ?? null;
  const highlightedCol = selected?.column ?? null;

  return (
    <div className="grid w-full grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-6">
      <Card className="overflow-hidden rounded-[16px] border-[#e2e8f0] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)]">
        <CardHeader className="border-b border-[#e2e8f0]">
          ...
          <CardDescription className="text-[#45556c]">
            Rows: I, A–H • Columns: 1–10 • Levels: 1–10
          </CardDescription>
        </CardHeader>
              ...
              <div className="h-10" />
                ...
                {ROWS.map((r) => (
                  <React.Fragment key={`row-${r}`}>
                    ...
                    <div
                      className={cn(
                        "flex h-[clamp(64px,7vh,120px)] items-center justify-center rounded-md border text-sm font-semibold transition-colors",
                    ...
                    {COLS.map((c) => {
                      ...
                      const level = isSelected ? selected?.level : null;
                      if (!valid) {
                        return (
                          <div
                            key={`${r}-${c}-invalid`}
                            aria-hidden="true"
                            className={cn(
                              "relative flex h-[clamp(64px,7vh,120px)] items-center justify-center rounded-md border",
                  ...
                  return (
                        <div
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter" || e.key === " ") {
                              e.preventDefault();
                              setSelected({ row: r, column: c, level: null });
                            }
                          }}
                          onClick={() => setSelected({ row: r, column: c, level: null })}
                          className={cn(
                            "relative h-[clamp(64px,7vh,120px)] rounded-md border bg-white transition-colors",
                        ...
                        >
                          ...
                          >
                            {LEVELS.map((lvl) => {
                              const active = isSelected && level === lvl;
                        return (
                                <button
                                  key={`${r}-${c}-lvl-${lvl}`}
                                  type="button"
                                  ...
                                  onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    setSelected({ row: r, column: c, level: lvl });
                                  }}
                                  className={cn(
                                ...
                                >
                                  <span className="pointer-events-none select-none">{lvl}</span>
                                </button>
                              );
                            })}
                          </div>
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              {selected ? (
                <Button
                  ...
                  className="h-8 rounded-[10px] bg-red-500 px-3 text-white hover:bg-red-600"
                >
                  Clear
                </Button>
              ) : null}
          ...
          <CardContent className="space-y-4 p-4">
            {selected ? (
              <>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    <div className="text-sm font-medium">Level / Bay (optional)</div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setSelected((prev) => (prev ? { ...prev, level: null } : prev))}
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {LEVELS.map((lvl) => {
                      const active = selected.level === lvl;
                      return (
                        <button
                          key={`panel-lvl-${lvl}`}
                          type="button"
                          onClick={() => setSelected((prev) => (prev ? { ...prev, level: lvl } : prev))}
                          className={cn(
                ...
                <div className="grid grid-cols-3 gap-2 text-sm">
                  ...
                  <div className="rounded-md border bg-background p-2">
                    <div className="text-xs text-muted-foreground">Bay</div>
                    <div className="font-semibold">{selected.level ?? "—"}</div>
                  </div>
                </div>
              ...
              <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">
                No selection yet. Click a grid cell (ex: <span className="font-mono">I-1</span>).
              </div>
            )}
          </CardContent>
        </Card>
